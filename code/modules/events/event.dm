...
/datum/event/New(datum/event_meta/EM, skeleton = FALSE, forced = FALSE)
	...
	if (alertadmins)
		message_admins(span_warning("[forced? "Зафоршенное" : "Случайное"] событие срабатает через 10 секунд: [EM.name] ([type]) (<a href='byond://?src=[UID()];cancel=1'>ОТМЕНИТЬ</a>)"))
...
/datum/event/Topic(href, href_list)
	...
	if(href_list["cancel"])
		...
		if(!forced && tgui_alert(usr, "Вы хотите, чтобы через 60 секунд был выбрано другое событие из этой категории(события, созданные не через панель событий или подсистему считаются мажорными)?", "", list("Да", "Нет")) == "Да")
